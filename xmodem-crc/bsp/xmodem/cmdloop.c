		...
		if ( ch == '\r' || ch == '\n')
		{
			...
			if ( getCommandSubStr(recvCommandBuf, 0, subStr) == 0 )
			{
				...
				else if ( strcmp(subStr, "run") == 0 )
				{
					...
					if ((*((unsigned char *)runAddr) == 'i') && 
						(*((unsigned char *)(runAddr+1)) == 'n') && 
						(*((unsigned char *)(runAddr+2)) == 'n') && 
						(*((unsigned char *)(runAddr+3)) == 'o')) {
						printf("SPL Header verify OK\r\n");
						printf("SPL size: %d \r\n", *((unsigned int *)(runAddr + 4)));
						crc = crc32(0, (unsigned char *)(runAddr + 12), *((unsigned int *)(runAddr + 4)));
						if (crc == *((unsigned int *)(runAddr + 8))) {
							printf("SPL CRC verify OK\r\n");
							funPtr = (unsigned int (*)(void))(runAddr+12);
							funPtr();	
						}
					}
				}
